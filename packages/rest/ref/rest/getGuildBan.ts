 ...
 */
import { type GetGuildBanRequestPath, type GetGuildBanRequestHeaders, type GuildBanResponse, type RateLimitError, type ErrorResponse } from '../discord.js';
...
export const rateLimit = {
    ...
    bucket(model: { ["guild_id"]: RouteModel["guild_id"] | string; }) {
        return `get /guilds/${model.guild_id}/bans/<any>` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = GetGuildBanRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as GuildBanResponse;
        }
