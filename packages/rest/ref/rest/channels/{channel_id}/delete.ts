 ...
 */
import { type DeleteChannelRequestPath, type DeleteChannelRequestHeaders, type DeleteChannelResponseJSON, type RateLimitError, type ErrorResponse } from '../../../types.js';
...
export const rateLimit = {
    ...
    bucket(model: { [P in "channel_id"]: RouteModel[P] | string; }) {
        return `delete /channels/${model.channel_id}` as const;
    }
} as const;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as DeleteChannelResponseJSON;
        }
