 ...
 */
import { type ListPinnedMessagesRequestPath, type ListPinnedMessagesRequestHeaders, type ListPinnedMessagesResponseJSON, type RateLimitError, type ErrorResponse } from '../../../../discord.js';
...
export const route = {
    ...
    get regex(){
        return /^\/channels\/(?<channel_id>.*?)\/pins$/i;
    },
    create(model: RouteModel) {
        return `/channels/${encodeURIComponent(model.channel_id)}/pins` as const satisfies `/${string}`;
    },
    test(url: `/${string}`) {
...
export const rateLimit = {
    ...
    bucket(model: { [P in "channel_id"]: RouteModel[P] | string; }) {
        return `get /channels/${model.channel_id}/pins` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = ListPinnedMessagesRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as ListPinnedMessagesResponseJSON;
        }
