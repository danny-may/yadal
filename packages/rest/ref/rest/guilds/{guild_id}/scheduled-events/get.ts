 ...
 */
import { type ListGuildScheduledEventsRequestPath, type ListGuildScheduledEventsRequestQuery, type ListGuildScheduledEventsRequestHeaders, type ListGuildScheduledEventsResponseJSON, type RateLimitError, type ErrorResponse } from '../../../../types.js';
...
export const rateLimit = {
    ...
    bucket(model: { [P in "guild_id"]: RouteModel[P] | string; }) {
        return `get /guilds/${model.guild_id}/scheduled-events` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = ListGuildScheduledEventsRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as ListGuildScheduledEventsResponseJSON;
        }
