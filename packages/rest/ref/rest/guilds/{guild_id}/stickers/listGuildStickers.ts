 ...
 */
import { type ListGuildStickersRequestPath, type ListGuildStickersRequestHeaders, type ListGuildStickersResponseJSON, type RateLimitError, type ErrorResponse } from '../../../../discord.js';
...
export const rateLimit = {
    ...
    bucket(model: { [P in "guild_id"]: RouteModel[P] | string; }) {
        return `get /guilds/${model.guild_id}/stickers` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = ListGuildStickersRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as ListGuildStickersResponseJSON;
        }
