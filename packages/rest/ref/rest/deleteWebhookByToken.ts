 ...
 */
import { type DeleteWebhookByTokenRequestPath, type DeleteWebhookByTokenRequestHeaders, type RateLimitError, type ErrorResponse } from '../discord.js';
...
export const rateLimit = {
    ...
    bucket(model: { ["webhook_id"]: RouteModel["webhook_id"] | string; ["webhook_token"]: RouteModel["webhook_token"] | string; }) {
        return `delete /webhooks/${model.webhook_id}/${model.webhook_token}` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = DeleteWebhookByTokenRequestHeaders;
