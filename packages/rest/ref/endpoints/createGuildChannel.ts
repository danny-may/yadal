 ...
 */
import { type CreateGuildChannelRequestPath, type RateLimitError, type GuildChannelResponse, type ErrorResponse, type CreateGuildChannelRequest } from '../discord.js';
...
export const route = {
    ...
    rateLimitBuckets(model: { ["guild_id"]: RouteModel["guild_id"] | string; }) {
        return ["global", `post /guilds/${model.guild_id}/channels`] as const;
    }
} as const;
Object.freeze(route);
...
export async function readResponse<R>(statusCode: number, contentType: string | undefined, content: R, resolve: (contentType: string, content: R) => Promise<unknown>): Promise<Response> {
    if (statusCode === 201) {
        if (contentType === "application/json") {
            return await resolve(contentType, content) as GuildChannelResponse;
        }
