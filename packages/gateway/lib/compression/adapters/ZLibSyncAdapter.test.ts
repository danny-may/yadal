...
import { ZLibSyncContextAdapter, ZLibSyncContextAdapterFactory } from './ZLibSyncAdapter.js';


describe(ZLibSyncContextAdapter.name, () => {
    it('Should decompress test case 1', async () => {
        // arrange
        const steps = [...deserializeTestCase(await fs.readFile(fileURLToPath(new URL('testData/ZLibSyncAdapter.0.dat', import.meta.url))))];
