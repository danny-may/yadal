 ...
 */
import { type GetStickerRequestPath, type GetStickerRequestHeaders, type GetStickerResponseJSON, type RateLimitError, type ErrorResponse } from '../../../types.js';
...
export const route = {
    ...
    get regex(){
        return /^\/stickers\/(?<sticker_id>.*?)$/i;
    },
    create(model: RouteModel) {
        return `/stickers/${encodeURIComponent(model.sticker_id)}` as const satisfies `/${string}`;
    },
    test(url: `/${string}`) {
...
export const rateLimit = {
    ...
    bucket(_?: { [P in never]: RouteModel[P] | string; }) {
        return `get /stickers/<any>` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = GetStickerRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as GetStickerResponseJSON;
        }
