 ...
 */
import { type GetGatewayRequestHeaders, type GatewayResponse, type RateLimitError, type ErrorResponse } from '../../types.js';
...
Object.freeze(query);
export type HeaderModel = GetGatewayRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as GatewayResponse;
        }
