 ...
 */
import { type GetChannelRequestPath, type GetChannelRequestHeaders, type GetChannelResponseJSON, type RateLimitError, type ErrorResponse } from '../../../types.js';
...
export const rateLimit = {
    ...
    bucket(model: { [P in "channel_id"]: RouteModel[P] | string; }) {
        return `get /channels/${model.channel_id}` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = GetChannelRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as GetChannelResponseJSON;
        }
