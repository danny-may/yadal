 ...
 */
import { type ListThreadMembersRequestPath, type ListThreadMembersRequestQuery, type ListThreadMembersRequestHeaders, type ListThreadMembersResponseJSON, type RateLimitError, type ErrorResponse } from '../../../../types.js';
...
export const route = {
    ...
    template: Object.freeze({
        raw: "/channels/{channel_id}/thread-members" as const,
        keys: Object.freeze(["channel_id"] as const),
        segments: Object.freeze(["/channels/","/thread-members"] as const)
    }),
...
export const rateLimit = {
    ...
    bucket(model: { [P in "channel_id"]: RouteModel[P] | string; }) {
        return `get /channels/${model.channel_id}/thread-members` as const;
    }
} as const;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as ListThreadMembersResponseJSON;
        }
