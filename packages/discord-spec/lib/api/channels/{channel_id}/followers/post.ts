 ...
 */
import { type FollowChannelRequestPath, type FollowChannelRequestHeaders, type ChannelFollowerResponse, type RateLimitError, type ErrorResponse, type FollowChannelRequestJSON } from '../../../../types.js';
...
export const route = {
    ...
    template: Object.freeze({
        raw: "/channels/{channel_id}/followers" as const,
        keys: Object.freeze(["channel_id"] as const),
        segments: Object.freeze(["/channels/","/followers"] as const)
    }),
...
export const rateLimit = {
    ...
    bucket(model: { [P in "channel_id"]: RouteModel[P] | string; }) {
        return `post /channels/${model.channel_id}/followers` as const;
    }
} as const;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as ChannelFollowerResponse;
        }
...
export function createBody(model: Body): { type: string; content: ArrayBufferView[]; } {
    return {
        ...
        content: [encoder.encode(JSON.stringify({
            "webhook_channel_id": model["webhook_channel_id" as keyof typeof model]
        }))]
    };
