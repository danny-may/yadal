 ...
 */
import { type CreateWebhookRequestPath, type CreateWebhookRequestHeaders, type GuildIncomingWebhookResponse, type RateLimitError, type ErrorResponse, type CreateWebhookRequestJSON } from '../../../../types.js';
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as GuildIncomingWebhookResponse;
        }
