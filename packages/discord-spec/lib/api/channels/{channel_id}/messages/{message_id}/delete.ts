 ...
 */
import { type DeleteMessageRequestPath, type DeleteMessageRequestHeaders, type RateLimitError, type ErrorResponse } from '../../../../../types.js';
...
export const rateLimit = {
    global: true,
    bucket(model: { [P in "channel_id" | "message_id"]: RouteModel[P] | string; }) {
        return `delete /channels/${model.channel_id}/messages/${((id: string) => {
            const age = Date.now() - Number((BigInt(id) >> 22n) + 1420070400000n /* Discord epoch */);
            if (age < 10000) return "group-0";
            if (age < 1209600000) return "group-1";
            return "group-rest";
