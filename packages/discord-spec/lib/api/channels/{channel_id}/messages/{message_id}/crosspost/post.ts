 ...
 */
import { type CrosspostMessageRequestPath, type CrosspostMessageRequestHeaders, type MessageResponse, type RateLimitError, type ErrorResponse } from '../../../../../../types.js';
import { DiscordRestError, DiscordRateLimitError } from '../../../../../../helpers.js';
export const name = "crosspostMessage";
export type RouteModel = CrosspostMessageRequestPath;
const routeRegex = /^\/channels\/(?<channel_id>.*?)\/messages\/(?<message_id>.*?)\/crosspost$/i;
export const route = {
    method: "POST",
    template: Object.freeze({
        raw: "/channels/{channel_id}/messages/{message_id}/crosspost" as const,
        keys: Object.freeze(["channel_id","message_id"] as const),
        segments: Object.freeze(["/channels/","/messages/","/crosspost"] as const)
    }),
    ...
    create(model: RouteModel) {
        return `/channels/${encodeURIComponent(model.channel_id)}/messages/${encodeURIComponent(model.message_id)}/crosspost` as const satisfies `/${string}`;
    },
    test(url: `/${string}`) {
...
Object.freeze(query);
export type HeaderModel = CrosspostMessageRequestHeaders;
