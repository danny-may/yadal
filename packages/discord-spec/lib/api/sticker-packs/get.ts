 ...
 */
import { type ListStickerPacksRequestHeaders, type StickerPackCollectionResponse, type RateLimitError, type ErrorResponse } from '../../types.js';
...
export const route = {
    ...
    template: Object.freeze({
        raw: "/sticker-packs" as const,
        keys: Object.freeze([] as const),
        segments: Object.freeze(["/sticker-packs"] as const)
    }),
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as StickerPackCollectionResponse;
        }
