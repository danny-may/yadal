 ...
 */
import { type LeaveGuildRequestPath, type LeaveGuildRequestHeaders, type RateLimitError, type ErrorResponse } from '../../../../../types.js';
import { DiscordRestError, DiscordRateLimitError } from '../../../../../helpers.js';
export const name = "leaveGuild";
export type RouteModel = LeaveGuildRequestPath;
const routeRegex = /^\/users\/@me\/guilds\/(?<guild_id>.*?)$/i;
export const route = {
    method: "DELETE",
    template: Object.freeze({
        raw: "/users/@me/guilds/{guild_id}" as const,
        keys: Object.freeze(["guild_id"] as const),
        segments: Object.freeze(["/users/@me/guilds/",""] as const)
    }),
    authentication: Object.freeze({
        "BotToken": Object.freeze([] as const)
    } as const),
    get regex(){
        return /^\/users\/@me\/guilds\/(?<guild_id>.*?)$/i;
    },
    create(model: RouteModel) {
        return `/users/@me/guilds/${encodeURIComponent(model.guild_id)}` as const satisfies `/${string}`;
    },
    test(url: `/${string}`) {
...
export const rateLimit = {
    ...
    bucket(model: { [P in "guild_id"]: RouteModel[P] | string; }) {
        return `delete /users/@me/guilds/${model.guild_id}` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = LeaveGuildRequestHeaders;
