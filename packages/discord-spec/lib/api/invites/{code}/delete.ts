 ...
 */
import { type InviteRevokeRequestPath, type InviteRevokeRequestHeaders, type InviteRevokeResponseJSON, type RateLimitError, type ErrorResponse } from '../../../types.js';
...
export const route = {
    ...
    template: Object.freeze({
        raw: "/invites/{code}" as const,
        keys: Object.freeze(["code"] as const),
        segments: Object.freeze(["/invites/",""] as const)
    }),
...
export const rateLimit = {
    ...
    bucket(_?: { [P in never]: RouteModel[P] | string; }) {
        return `delete /invites/<any>` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = InviteRevokeRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as InviteRevokeResponseJSON;
        }
