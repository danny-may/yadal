 ...
 */
import { type InviteResolveRequestPath, type InviteResolveRequestQuery, type InviteResolveRequestHeaders, type InviteResolveResponseJSON, type RateLimitError, type ErrorResponse } from '../../../types.js';
...
export const rateLimit = {
    ...
    bucket(_?: { [P in never]: RouteModel[P] | string; }) {
        return `get /invites/<any>` as const;
    }
} as const;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as InviteResolveResponseJSON;
        }
