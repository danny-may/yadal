 ...
 */
import { type GetGuildWebhooksRequestPath, type GetGuildWebhooksRequestHeaders, type GetGuildWebhooksResponseJSON, type RateLimitError, type ErrorResponse } from '../../../../types.js';
...
export const route = {
    ...
    template: Object.freeze({
        raw: "/guilds/{guild_id}/webhooks" as const,
        keys: Object.freeze(["guild_id"] as const),
        segments: Object.freeze(["/guilds/","/webhooks"] as const)
    }),
...
export const rateLimit = {
    ...
    bucket(model: { [P in "guild_id"]: RouteModel[P] | string; }) {
        return `get /guilds/${model.guild_id}/webhooks` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = GetGuildWebhooksRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as GetGuildWebhooksResponseJSON;
        }
