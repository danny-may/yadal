 ...
 */
import { type SetGuildMfaLevelRequestPath, type SetGuildMfaLevelRequestHeaders, type GuildMFALevelResponse, type RateLimitError, type ErrorResponse, type SetGuildMfaLevelRequestJSON } from '../../../../types.js';
...
export const route = {
    ...
    template: Object.freeze({
        raw: "/guilds/{guild_id}/mfa" as const,
        keys: Object.freeze(["guild_id"] as const),
        segments: Object.freeze(["/guilds/","/mfa"] as const)
    }),
...
export const rateLimit = {
    ...
    bucket(model: { [P in "guild_id"]: RouteModel[P] | string; }) {
        return `post /guilds/${model.guild_id}/mfa` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = SetGuildMfaLevelRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as GuildMFALevelResponse;
        }
...
export function createBody(model: Body): { type: string; content: ArrayBufferView[]; } {
    return {
        ...
        content: [encoder.encode(JSON.stringify({
            "level": model["level" as keyof typeof model]
        }))]
    };
