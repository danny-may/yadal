 ...
 */
import { type DeleteGuildTemplateRequestPath, type DeleteGuildTemplateRequestHeaders, type GuildTemplateResponse, type RateLimitError, type ErrorResponse } from '../../../../../types.js';
import { DiscordRestError, DiscordRateLimitError } from '../../../../../helpers.js';
export const name = "deleteGuildTemplate";
export type RouteModel = DeleteGuildTemplateRequestPath;
const routeRegex = /^\/guilds\/(?<guild_id>.*?)\/templates\/(?<code>.*?)$/i;
export const route = {
    method: "DELETE",
    template: Object.freeze({
        raw: "/guilds/{guild_id}/templates/{code}" as const,
        keys: Object.freeze(["guild_id","code"] as const),
...
export const rateLimit = {
    ...
    bucket(model: { [P in "guild_id"]: RouteModel[P] | string; }) {
        return `delete /guilds/${model.guild_id}/templates/<any>` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = DeleteGuildTemplateRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as GuildTemplateResponse;
        }
