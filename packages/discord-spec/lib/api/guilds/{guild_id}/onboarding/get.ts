 ...
 */
import { type GetGuildsOnboardingRequestPath, type GetGuildsOnboardingRequestHeaders, type UserGuildOnboardingResponse, type RateLimitError, type ErrorResponse } from '../../../../types.js';
import { DiscordRestError, DiscordRateLimitError } from '../../../../helpers.js';
export const name = "getGuildsOnboarding";
export type RouteModel = GetGuildsOnboardingRequestPath;
const routeRegex = /^\/guilds\/(?<guild_id>.*?)\/onboarding$/i;
export const route = {
    method: "GET",
    template: Object.freeze({
        raw: "/guilds/{guild_id}/onboarding" as const,
        keys: Object.freeze(["guild_id"] as const),
        segments: Object.freeze(["/guilds/","/onboarding"] as const)
    }),
...
export const rateLimit = {
    ...
    bucket(model: { [P in "guild_id"]: RouteModel[P] | string; }) {
        return `get /guilds/${model.guild_id}/onboarding` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = GetGuildsOnboardingRequestHeaders;
...
export type Response = UserGuildOnboardingResponse;
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as UserGuildOnboardingResponse;
        }
