 ...
 */
import { type CreateAutoModerationRuleRequestPath, type CreateAutoModerationRuleRequestHeaders, type CreateAutoModerationRuleResponseJSON, type RateLimitError, type ErrorResponse, type CreateAutoModerationRuleRequestJSON } from '../../../../../types.js';
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as CreateAutoModerationRuleResponseJSON;
        }
