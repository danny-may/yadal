 ...
 */
import { type GetGuildWidgetRequestPath, type GetGuildWidgetRequestHeaders, type WidgetResponse, type RateLimitError, type ErrorResponse } from '../../../../types.js';
import { DiscordRestError, DiscordRateLimitError } from '../../../../helpers.js';
export const name = "getGuildWidget";
export type RouteModel = GetGuildWidgetRequestPath;
const routeRegex = /^\/guilds\/(?<guild_id>.*?)\/widget\.json$/i;
export const route = {
    method: "GET",
    template: Object.freeze({
        raw: "/guilds/{guild_id}/widget.json" as const,
        keys: Object.freeze(["guild_id"] as const),
        segments: Object.freeze(["/guilds/","/widget.json"] as const)
    }),
    ...
    get regex(){
        return /^\/guilds\/(?<guild_id>.*?)\/widget\.json$/i;
    },
    create(model: RouteModel) {
        return `/guilds/${encodeURIComponent(model.guild_id)}/widget.json` as const satisfies `/${string}`;
    },
    test(url: `/${string}`) {
...
export const rateLimit = {
    ...
    bucket(model: { [P in "guild_id"]: RouteModel[P] | string; }) {
        return `get /guilds/${model.guild_id}/widget.json` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = GetGuildWidgetRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as WidgetResponse;
        }
