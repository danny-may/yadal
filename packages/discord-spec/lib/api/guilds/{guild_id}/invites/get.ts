 ...
 */
import { type ListGuildInvitesRequestPath, type ListGuildInvitesRequestHeaders, type ListGuildInvitesResponseJSON, type RateLimitError, type ErrorResponse } from '../../../../types.js';
import { DiscordRestError, DiscordRateLimitError } from '../../../../helpers.js';
export const name = "listGuildInvites";
export type RouteModel = ListGuildInvitesRequestPath;
const routeRegex = /^\/guilds\/(?<guild_id>.*?)\/invites$/i;
export const route = {
    method: "GET",
    template: Object.freeze({
        raw: "/guilds/{guild_id}/invites" as const,
        keys: Object.freeze(["guild_id"] as const),
        segments: Object.freeze(["/guilds/","/invites"] as const)
    }),
    authentication: Object.freeze({
        "BotToken": Object.freeze([] as const)
    } as const),
    get regex(){
        return /^\/guilds\/(?<guild_id>.*?)\/invites$/i;
    },
    create(model: RouteModel) {
        return `/guilds/${encodeURIComponent(model.guild_id)}/invites` as const satisfies `/${string}`;
    },
    test(url: `/${string}`) {
...
export const rateLimit = {
    ...
    bucket(model: { [P in "guild_id"]: RouteModel[P] | string; }) {
        return `get /guilds/${model.guild_id}/invites` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = ListGuildInvitesRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as ListGuildInvitesResponseJSON;
        }
