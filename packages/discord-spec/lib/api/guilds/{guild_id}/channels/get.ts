 ...
 */
import { type ListGuildChannelsRequestPath, type ListGuildChannelsRequestHeaders, type ListGuildChannelsResponseJSON, type RateLimitError, type ErrorResponse } from '../../../../types.js';
...
export const route = {
    ...
    template: Object.freeze({
        raw: "/guilds/{guild_id}/channels" as const,
        keys: Object.freeze(["guild_id"] as const),
        segments: Object.freeze(["/guilds/","/channels"] as const)
    }),
...
export const rateLimit = {
    ...
    bucket(model: { [P in "guild_id"]: RouteModel[P] | string; }) {
        return `get /guilds/${model.guild_id}/channels` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = ListGuildChannelsRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as ListGuildChannelsResponseJSON;
        }
