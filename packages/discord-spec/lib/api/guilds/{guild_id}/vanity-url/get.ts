 ...
 */
import { type GetGuildVanityUrlRequestPath, type GetGuildVanityUrlRequestHeaders, type VanityURLResponse, type RateLimitError, type ErrorResponse } from '../../../../types.js';
...
export const route = {
    ...
    template: Object.freeze({
        raw: "/guilds/{guild_id}/vanity-url" as const,
        keys: Object.freeze(["guild_id"] as const),
        segments: Object.freeze(["/guilds/","/vanity-url"] as const)
    }),
...
export const rateLimit = {
    ...
    bucket(model: { [P in "guild_id"]: RouteModel[P] | string; }) {
        return `get /guilds/${model.guild_id}/vanity-url` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = GetGuildVanityUrlRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as VanityURLResponse;
        }
