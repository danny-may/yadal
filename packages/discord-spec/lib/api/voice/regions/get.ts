 ...
 */
import { type ListVoiceRegionsRequestHeaders, type ListVoiceRegionsResponseJSON, type RateLimitError, type ErrorResponse } from '../../../types.js';
import { DiscordRestError, DiscordRateLimitError } from '../../../helpers.js';
export const name = "listVoiceRegions";
export type RouteModel = {

};
const routeRegex = /^\/voice\/regions$/i;
export const route = {
    method: "GET",
    template: Object.freeze({
        raw: "/voice/regions" as const,
        keys: Object.freeze([] as const),
        segments: Object.freeze(["/voice/regions"] as const)
    }),
    authentication: Object.freeze({
        "BotToken": Object.freeze([] as const)
    } as const),
    get regex(){
        return /^\/voice\/regions$/i;
    },
    create(_?: RouteModel) {
        return `/voice/regions` as const satisfies `/${string}`;
    },
    test(url: `/${string}`) {
...
Object.freeze(query);
export type HeaderModel = ListVoiceRegionsRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as ListVoiceRegionsResponseJSON;
        }
