 ...
 */
import { type UpdateWebhookRequestPath, type UpdateWebhookRequestHeaders, type UpdateWebhookResponseJSON, type RateLimitError, type ErrorResponse, type UpdateWebhookRequestJSON } from '../../../types.js';
...
export const route = {
    ...
    template: Object.freeze({
        raw: "/webhooks/{webhook_id}" as const,
...
export const rateLimit = {
    global: true,
    bucket(model: { [P in "webhook_id"]: RouteModel[P] | string; }) {
        return `patch /webhooks/${model.webhook_id}` as const;
    }
} as const;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as UpdateWebhookResponseJSON;
        }
...
export function createBody(model: Body): { type: string; content: ArrayBufferView[]; } {
    return {
        ...
        content: [encoder.encode(JSON.stringify({
            ...
            "avatar": model["avatar" as keyof typeof model],
