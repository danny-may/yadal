 ...
 */
import { type GetWebhookRequestPath, type GetWebhookRequestHeaders, type GetWebhookResponseJSON, type RateLimitError, type ErrorResponse } from '../../../types.js';
...
export const rateLimit = {
    ...
    bucket(model: { [P in "webhook_id"]: RouteModel[P] | string; }) {
        return `get /webhooks/${model.webhook_id}` as const;
    }
} as const;
...
Object.freeze(query);
export type HeaderModel = GetWebhookRequestHeaders;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as GetWebhookResponseJSON;
        }
