 ...
 */
import { type GetWebhookByTokenRequestPath, type GetWebhookByTokenRequestHeaders, type GetWebhookByTokenResponseJSON, type RateLimitError, type ErrorResponse } from '../../../../types.js';
...
export const rateLimit = {
    ...
    bucket(model: { [P in "webhook_id" | "webhook_token"]: RouteModel[P] | string; }) {
        return `get /webhooks/${model.webhook_id}/${model.webhook_token}` as const;
    }
} as const;
...
export async function readResponse(statusCode: number, contentType: string | undefined, content: () => Promise<ArrayBufferView>): Promise<Response> {
    if (statusCode === 200) {
        if (contentType === "application/json") {
            return JSON.parse(decode(await content())) as GetWebhookByTokenResponseJSON;
        }
